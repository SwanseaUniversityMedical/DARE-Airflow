...
def process_s3_object(bucket, key, etag, action):
    ...
    attribs, templates, duckdb_params, process, action, default_used = get_instructions(event['dir_name'])
    # use bucket settings if no Dataset settings
    if default_used:
        attribs, templates, duckdb_params, process, action, default_used = get_instructions(f"BUCKET-{bucket}")

    logging.info(f'default used = {default_used}')
    ...
    # should we load data ?
    if action != constants.process_s3_option_whatif and \
            process != constants.process_s3_formoption_no and \
            (action == constants.process_s3_option_manual or
             (process == constants.process_s3_formoption_yesauto and
              (action == constants.process_s3_option_load or
               action == constants.process_s3_option_default)
              )
             ):
