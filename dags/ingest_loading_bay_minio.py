...
with DAG(
    ...
    def process_event(message):
        ...
        # (two methods separated so the first one can be reused)
        bucket, key, etag = unpack_minio_event(message)
        logging.info(f'unpacked basic = bucket: {bucket}, key: {key}, etag: {etag}')

        # NOTE: done this way as manual trigger could be on another
        # DAG listening to RabbitMQ with message that only
        # has these three variables
